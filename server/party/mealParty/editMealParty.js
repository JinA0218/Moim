...
//edit meal-party
const editMealParty=function (req, res){
    ...
    debug(`POST /edit-party/\tmeal-party`);
    ...
    if(party_head!==undefined){
        ...
        connection.query('select userid from users where userid=?', [party_head], async(error, rows, field)=>{
            if(error){
                // Query error.
                debug("editMealParty_checking userid failed due to query error 0");
                debug(error.message);
                ...
                if (party_head!==undefined&&party_id!==undefined&& party_name !== undefined && has_place!==undefined && place1!==undefined && place2!==undefined &&
                    ...
                    party_time!==undefined && maximum_count!==undefined && detailed_description!==undefined
                        ...
                        connection.query('select party_id, current_count from meal_party where party_id=?', [party_id], async (error, rows, field) =>{
                            if (error) {
                                // Query error.
                                debug("editMealParty failed due to query error 1");
                                debug(error.message);
                                res.status(400).send(error.message);
                            }
                            else if (rows.length == 0) {
                                ...
                                if (maximum_count<current_count){
                                    ...
                                    connection.query('update meal_party set party_head=?, party_name=?, has_place=?, place1=?, place2=?, place3=?, meal_type=?, outside=?, party_date=?, party_time=?, maximum_count=?, count_difference=?, detailed_description=? where party_id=?',
                                    ...
                                    async(error, rows, field)=> {
                                                if (error) {
                                                    // Query error again..
                                                    debug("editMealParty failed due to query error 2");
                                                    debug(error.message);
                                                    res.status(400).send(error.message);
                                                }
                                                else {
                                                    debug(`editMealParty PARTY ID : ${party_id}, successfully edited.`);
