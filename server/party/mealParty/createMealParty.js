...
const createMealParty=function (req, res){
    ...
    debug(`POST /create-party/\tmealParty`);
    ...
    if(party_head!==undefined){
        ...
        connection.query('select userid, username from users where userid=?', [party_head], async(error, rows, field)=>{
            if(error){
                // Query error.
                debug("createMealParty_checking userid failed due to query error 0");
                debug(error.message);
                ...
                if (party_id !== undefined &&party_head!==undefined&& party_name !== undefined && has_place!==undefined && place1!==undefined && place2!==undefined &&
                    ...
                    party_time!==undefined && current_count!==undefined && maximum_count!==undefined && detailed_description!==undefined && count_difference!==undefined
                        ...
                        connection.query('select party_id from meal_party where party_id=?', [party_id], async (error, rows, field) =>{
                    if (error) {
                        // Query error.
                        debug("createMealParty failed due to query error 1");
                        debug(error.message);
                        res.status(400).send(error.message);
                    }
                    else if (rows.length > 0) {
                        ...
                        if (maximum_count<current_count){
                            ...
                            connection.query('insert into meal_party(party_id, party_head, party_name, has_place, place1, place2, place3, meal_type, outside, party_date, party_time, current_count, maximum_count, count_difference, detailed_description)\
                            values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
                            ...
                            async(error, rows, field)=> {
                                        if (error) {
                                            // Query error again..
                                            debug("createMealParty failed due to query error 2");
                                            debug(error.message);
                                            res.status(400).send(error.message);
                                        }
                                        else {
                                            
                                            debug(`createMealParty PARTY ID : ${party_id}, successfully registered.`);
                                            global_id+=1;
