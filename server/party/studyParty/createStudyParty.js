...
const createStudyParty=function (req, res){
    ...
    debug(`POST /create-party/\tstudyParty`);
    ...
    if(party_head!==undefined){
        ...
        connection.query('select userid, username from users where userid=?', [party_head], async(error, rows, field)=>{
            if(error){
                // Query error.
                debug("createStudyParty_checking userid failed due to query error 0");
                debug(error.message);
                ...
                if (party_id !== undefined &&party_head!==undefined&& party_name !== undefined && has_place!==undefined && place1!==undefined && place2!==undefined &&
                    place3!==undefined && current_count!==undefined && maximum_count!==undefined && detailed_description!==undefined && count_difference!==undefined
                        ...
                        connection.query('select party_id from study_party where party_id=?', [party_id], async (error, rows, field) =>{
                    if (error) {
                        // Query error.
                        debug("createStudyParty failed due to query error 1");
                        debug(error.message);
                        res.status(400).send(error.message);
                    }
                    else if (rows.length > 0) {
                        ...
                        if (maximum_count<current_count){
                            ...
                            connection.query('insert into study_party(party_id, party_head, party_name, has_place, place1, place2, place3, current_count, maximum_count, count_difference, detailed_description)\
                            values (?,?,?,?,?,?,?,?,?,?,?)',
                            ...
                            async(error, rows, field)=> {
                                        if (error) {
                                            // Query error again..
                                            debug("createStudyParty failed due to query error 2");
                                            debug(error.message);
                                            res.status(400).send(error.message);
                                        }
                                        else {
                                            
                                            debug(`createStudyParty PARTY ID : ${party_id}, successfully registered.`);
                                            global_id+=1;
