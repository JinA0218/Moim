...
//edit night-meal-party
const editNightMealParty=function (req, res){
    ...
    debug(`POST /edit-party/\tnight-meal-party`);
    ...
    if(party_head!==undefined){
        ...
        connection.query('select userid from users where userid=?', [party_head], async(error, rows, field)=>{
            if(error){
                // Query error.
                debug("editNightMealParty_checking userid failed due to query error 0");
                debug(error.message);
                ...
                if (party_head!==undefined&&party_id!==undefined&& party_name !== undefined && has_place!==undefined && place1!==undefined && place2!==undefined &&
                    place3!==undefined && meal_type!==undefined && outside!==undefined && party_date!==undefined &&
                    party_time!==undefined && maximum_count!==undefined && detailed_description!==undefined
                    ) {
                        connection.query('select party_id, current_count from night_meal_party where party_id=?', [party_id], async (error, rows, field) =>{
                            if (error) {
                                // Query error.
                                debug("editNightMealParty failed due to query error 1");
                                debug(error.message);
                                res.status(400).send(error.message);
                            }
                            else if (rows.length == 0) {
                                ...
                                if (maximum_count<current_count){
                                    ...
                                    connection.query('update night_meal_party set party_head=?, party_name=?, has_place=?, place1=?, place2=?, place3=?, meal_type=?, outside=?, party_date=?, party_time=?, maximum_count=?, count_difference=?, detailed_description=? where party_id=?',
                                    [party_head, party_name,has_place,place1,place2,place3,meal_type,outside,party_date,party_time,maximum_count,count_difference, detailed_description, party_id],
                                    async(error, rows, field)=> {
                                                if (error) {
                                                    // Query error again..
                                                    debug("editNightMealParty failed due to query error 2");
                                                    debug(error.message);
                                                    res.status(400).send(error.message);
                                                }
                                                else {
                                                    debug(`editNightMealParty PARTY ID : ${party_id}, successfully edited.`);
